...
class DuelingDDeepQNetwork(nn.Module):
    def __init__(self, lr, n_actions, name, input_dims, chkpt_dir):
        super(DuelingDDeepQNetwork, self).__init__()
...
class DuelingDDQNAgent(object):
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims, mem_size, batch_size, eps_min=0.01, eps_dec=5e-7,
        ...
        self.q_eval = DuelingDDeepQNetwork(self.lr, self.n_actions,
                                   input_dims=self.input_dims,
                                   name=self.env_name+'_'+self.algo+'_q_eval',
                                   chkpt_dir=self.chkpt_dir)

        self.q_next = DuelingDDeepQNetwork(self.lr, self.n_actions,
                                   input_dims=self.input_dims,
    ...
    def learn(self):
        if self.memory.mem_ctr > self.batch_size:
            ...
            Vs_eval, As_eval = self.q_eval.forward(states_)

            q_pred = T.add(Vs, (As - As.mean(dim=1, keepdim=True)))[indices, actions]
            q_next = T.add(Vs_, (As_ - As_.mean(dim=1, keepdim=True)))
...
if __name__ == '__main__':
    ...
    agent = DuelingDDQNAgent(gamma=0.99, epsilon=1.0, lr=0.0001, input_dims=env.observation_space.shape,
                     ...
                     replace=1000, eps_dec=1e-5, chkpt_dir='models/', algo='DuelingDDQNAgent',
